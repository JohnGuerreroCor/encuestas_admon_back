...
public class UsuarioServiceImpl implements UserDetailsService, IUsuarioService {
	...
	public UserDetails loadUserByUsername(String usuarioDb) throws UsernameNotFoundException {
		HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes())
				.getRequest();
		String segundaClave = request.getParameter("clave2");

		// VALIDAR SI EL USUARIO EXISTE EN EL SISTEMA
		if (!usuariodao.validarUsuario(usuarioDb)) {
			logger.error("Error, no existe el usuario '" + usuarioDb + "' en el sistema!!!");
			throw new DisabledException("No existe el usuario en el sistema.");
		}

		// OBTENER INFORMACIÓN DEL USUARIO
		Usuario usuario = usuariodao.buscarUsuario(usuarioDb);
		...
		segundaClaveComponent.setClave(usuarioDb, SegundaClaveReal);
		segundaClaveComponent.setUser(usuarioDb, usuario.getUserdb());
	...
	public Usuario buscarUsuario(String usuarioDb) {
		return usuariodao.buscarUsuario(usuarioDb);
	}
