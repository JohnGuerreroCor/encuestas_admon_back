...
public class InicioSesionDaoImpl implements IInicioSesionDao {
    ...
    public String obtenerTokenInicioSesion(String atributos, String usuarioDb) {
        ...
        String token = null;
        try {
            token = jdbcTemplateLogin.queryForObject("SELECT token.getTokenInicioSesion(?)", new Object[] { atributos }, String.class);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return token;
    ...
    public String urlTokenPeticion(String usuarioDb) {
        ...
        String url = null;
        try {
            url = jdbcTemplateLogin.queryForObject("SELECT wep_valor FROM dbo.web_parametro wp where wp.wep_codigo = 377", String.class);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return url;
